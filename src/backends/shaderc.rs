use crate::{CompilationFeedback, InputSourceLanguage, OptimizationLevel, ShaderCompilationConfig, ShaderKind, TargetEnvironmentType, TargetSpirvVersion};
use shaderc;

pub(crate) fn compile(src: &str, path: Option<&str>, cfg: &ShaderCompilationConfig) -> Result<CompilationFeedback, String> {
    use std::path::Path;
    ...
    let (target_env, vulkan_version) = match (cfg.env_ty, cfg.spv_ver) {
        (TargetEnvironmentType::Vulkan, TargetSpirvVersion::Spirv1_0) => (shaderc::TargetEnv::Vulkan, shaderc::EnvVersion::Vulkan1_0),
        (TargetEnvironmentType::Vulkan, TargetSpirvVersion::Spirv1_3) => (shaderc::TargetEnv::Vulkan, shaderc::EnvVersion::Vulkan1_1),
        (TargetEnvironmentType::Vulkan, TargetSpirvVersion::Spirv1_5) => (shaderc::TargetEnv::Vulkan, shaderc::EnvVersion::Vulkan1_2),
    ...
    let mut opt = shaderc::CompileOptions::new().ok_or("cannot create `shaderc::CompileOptions`")?;
    ...
    let feedback = CompilationFeedback { spv };
    Ok(feedback)
}
